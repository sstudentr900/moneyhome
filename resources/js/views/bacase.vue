<script setup>
  ...
  import { bacase,bacaseadd,bacaseedit,bacaseeditpost,bacasesort,bacasedelete } from "../composables/useApi"
  import { useRoute,useRouter } from 'vue-router'
  const route = useRoute()
  const router = useRouter()
  const pagename = ref('bacase')
  ...
  const searchFn = async(now=route.params.id)=>{
    ...
    try{
      ...
      console.log('bacase error',error)
      notify({title: error});
  ...
  const addFn = async(values,actions)=>{
    ...
    try{
      ...
      const result = await bacaseadd(values);
  ...
  const editpostFn = async(values,actions)=>{
    ...
    try{
      ...
      loadingShow.value = true
      const result = await bacaseeditpost(id,values);
      loadingShow.value = false
      if(result.status){
        notify({title: result.message});
        // 成功去管員
  ...
  const editFn = async()=>{
    try{
      const id = route.params.id
      const result = await bacaseedit(id);
  ...
  const deletFn = async(id,searchfn,cancelfn) => {
    try{
      loadingShow.value = true
      const result = await bacasedelete(id);
      loadingShow.value = false
      if(result.status){
        notify({title: "刪除成功"});
