<script setup>
  ...
  const eyeClick = ()=>{
    // console.log('eyeClick',passwordType.value)
...
<template>
  <div class="inputDiv_dif_fdc">
    <div class="label_text16">
      <label for="password">{{props.text}} <span class="_coRed" v-if="props.required">*</span></label>
    </div>
    <div class="input_wi100_por">
      <div class="eye_poa" @click="eyeClick">
        <div class="close" v-if="passwordType=='password'">
      ...
      ></v-field>
      <error-message name="password" class="_formError"></error-message>
    </div>
  </div>
</template>
<style scoped>
