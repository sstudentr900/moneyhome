  ...
  <div class="input">
    <div>
      <div class="publicImg">
        ...
        @if(old($id))
          @php
            $imgSaveSrc = isset($imgSaveSrc)&&$imgSaveSrc?$imgSaveSrc:env('MIX_IMGPREFIX');
      ...
      @if(isset($remark))<p class="_fs14_coGray700_mat8">{{ $remark }}</p>@endif
    </div>
  </div>
  {{-- css --}}
  @once
...
@elseif ($type=='file5')  
  ...
  <div class="publicAppendix" data-target="fileDiv">
    <label data-target='add'>{{ $label }}@if(isset($require))<span class="puplicRequired">*</span>@endif<svg xmlns="http://www.w3.org/2000/svg" width="537.947px" height="537.947px" viewBox="0 0 537.947 537.947"><path d="M268.974,0C120.411,0,0,120.411,0,268.974c0,148.563,120.411,268.973,268.974,268.973s268.973-120.334,268.973-268.973
		C537.947,120.334,417.537,0,268.974,0z M394.51,286.033c0,11.857-9.715,21.496-21.496,21.496h-64.566v64.566
		c0,11.857-9.715,21.496-21.496,21.496h-35.878c-11.857,0-21.497-9.715-21.497-21.496v-64.566h-64.566
		c-11.857,0-21.497-9.715-21.497-21.496v-35.878c0-11.857,9.715-21.497,21.497-21.497h64.566v-64.566
		c0-11.857,9.715-21.496,21.497-21.496h35.878c11.857,0,21.496,9.715,21.496,21.496v64.566h64.566
...
@elseif ($type=='releases')
  ...
  @push('customStyle')
    <style>
      ...
      [class^='publicRadio'] label{
        margin-left: 4px
      }
      [class^='publicRadio'] .radio{
        display: flex;
        align-items: center;
      }
      [class^='publicRadio'] .radio+.radio{
        margin-left: 16px;
      }
    </style>
  @endpush
  @endonce
@elseif ($type=='releases2')
...
@elseif ($type=='select')
  ...
  {{-- 
    ...
    @include(
      'layouts.input',
      ['type'=>'select','id'=>'case_source','label'=>'個案來源','require'=>true,'options'=>array('1'=>'1','2'=>'2')]
    ) 
  --}}
@elseif ($type=='textarea')
  ...
  <div class="input">
    ...
    placeholder="請輸入{{ isset($place)?$place:$label }}"
    required="{{isset($require)?'required':''}}"
    >{{ old($id)?old($id):(!$errors->any() && isset($datas)?$datas->$id:'') }}</textarea>
    @error($id)
      <div class="puplicError">{{ $message }}</div>
    @enderror
  </div>
...
@elseif ($type=='verify')
  <label for="{{ $id }}" class="label">{{ $label }}@if(isset($require))<span class="puplicRequired">*</span>@endif</label>
  <div class="inputDiv_dif_gap16">
    <div class="input_wi60">
      <input 
      type="text" 
      id="{{ $id }}" 
      name="{{ isset($name)?$name:$id }}" 
    ...
    <div class="img_wi40">
      {{-- <img src="{{ URL::asset('img/login01.png') }}" alt=""> --}}
      <img src="{{ URL::asset(env('MIX_IMGPREFIX')).'/login01.png?'.rand() }}">
      {{-- <img src="{{ URL::asset('images/').'/'.$CourseNews->cover}}" alt="">
      <img src="login01.png" alt=""> --}}
    </div>
  </div> 
